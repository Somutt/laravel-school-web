        ...
        <form onSubmit={submitClass}>
            <div className="flex flex-col justify-center items-center w-full">
                <div className="">
                    ...
                    <select id="professor" name="professor" className="w-64" value={data.professor} onChange={ (e) => setData('professor', e.target.value)}>
                        {professors.map( (p) => <option key={p.id} >{p.name}</option> )}
                    </select>
                    <InputLabel htmlFor="room" value="Select Room" className="mt-2"/>
                    <select id="room" name="room" className="w-64" value={data.room} onChange={ (e) => setData('room', e.target.value) } >
